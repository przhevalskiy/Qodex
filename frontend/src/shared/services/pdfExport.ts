...
/**
 * Export full discussion history to PDF.
 * Lists every discussion with its title, timestamps, and chat URL.
 */
export async function exportHistoryToPDF(discussions: Discussion[]): Promise<void> {
  ...
  pdf.setFont('helvetica', 'normal');
  pdf.setTextColor(128, 128, 128);
  pdf.text(`Exported: ${new Date().toLocaleString()}  |  Total discussions: ${discussions.length}`, margin, yPosition);
  yPosition += 8;
  ...
  for (let i = 0; i < discussions.length; i++) {
    ...
    const wrappedTitle = pdf.splitTextToSize(`${i + 1}. ${title}`, contentWidth);
  ...
  const dateStr = new Date().toISOString().split('T')[0];
  pdf.save(`qodex-history-${dateStr}.pdf`);
}
